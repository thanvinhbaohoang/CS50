...
if [[ $(cut -d "," -f 5 vaccine.csv| grep -q "$1") == 0 ]]; then
    		echo >&2 "State '$1' does not exist!!"
		exit 1
	else
#Find the State's Vaccination Number From Latest_date
	echo $1; grep $latest_date vaccine.csv| grep $1| awk -F ',' '{print $7}'| awk 'NR>1'| awk -F, '{ sum += $1; } END { print sum}'
	exit 0
	fi
fi
...
if [[ $(cut -d "," -f 1 vaccine.csv| grep -c "$2") == 0 ]]; then
    	echo  >&2"This Date does not exist!!"
	exit 1
else 
	echo $1,$2; grep $2 vaccine.csv| grep $1| awk -F ',' '{print $7}'| awk 'NR>1'| awk -F, '{ sum += $1; } END { print sum}'
	exit 0
